...
exports.getMoneyBookRangeData = async (start, end) => {
  ...
  try {
    const [data] = await connection.query(
      `select nt.id, nt.date, nt.content, nt.amount, nt.category_id, ct.name as category, nt.payment_method_id, nt.payment_method, ct.isIncome from (select mb.id, mb.date, mb.content, mb.amount, mb.category_id, pm.name as payment_method, mb.payment_method_id from money_book as mb left join payment_method as pm on mb.payment_method_id = pm.id) as nt left join category as ct on nt.category_id = ct.id where DATE_FORMAT(date, '%Y%m') BETWEEN ? AND ? order by date desc`,
      [start, end],
    );
    return data;
  } catch (err) {
